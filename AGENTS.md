...
plugins/claude-code/             plugin wrapper: commands/deploy.md + skills symlinks
...
- `new_workstream` (one message; phone button arms→confirms): marker ← convo transcript line count; spawn planner (`/remote-workstreams:role-stint-plan convo=… since_line=… output=…`); poll for its output file (2s interval, 300s budget); kill the planner; launch IMMEDIATELY — **the plan is trusted, never shown for review**. Plan's first line is `Stint: <title>` (hard contract with role-stint-plan; the title is all the user ever sees) → window `ws-<slug>`, spawned with `/role-root` + the workstream settings file; full plan pasted only after `_await_ready` sees the role greeting in the transcript.
...
- **Skills**: Codex invokes skills as `$skill-name` in any prompt, including the CLI's trailing prompt arg. It discovers them in `~/.codex/skills` (proven working on 0.142.5 — the Jul 7 session saw the harness roles there; current docs also list `~/.agents/skills` and repo-level `.agents/skills`). `$role-root` comes from the harness codex adapter (`~/Documents/harness/codex/install.sh`); `$role-convo` is linked into `~/.codex/skills` by the deploy skill's Engines step (done on this machine 2026-07-08). Codex reads global instructions from `~/.codex/AGENTS.md` (the harness adapter generates it from CLAUDE.md) before every session.
- **Codex plugin** (`plugins/codex/` + root `.agents/plugins/marketplace.json`): `codex plugin marketplace add <repo-or-path>` then `codex plugin add remote-workstreams@remote-workstreams` — both non-interactive, no model use; verified installed+discovered on this machine via `codex debug prompt-input` (renders the model-visible skill list without a session). Two format traps, both verified live: the marketplace manifest's source key is `{"source": "local", ...}` — NOT `{"type": ...}` as in older examples — and the plugin cache copier DROPS symlinks (an out-of-tree skill symlink installs as an empty dir), which is why the codex plugin carries a real copy of the deploy skill, pinned byte-for-byte by `tests/test_plugins.py`.

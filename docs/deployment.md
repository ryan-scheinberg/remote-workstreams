...
1. Run `skills/deploy-rw/scripts/check.sh "$PWD"` and fix missing `uv`, `tmux`, an agent CLI, or Tailscale first.
...
5. Run `skills/deploy-rw/scripts/install_service.sh "$PWD"`. It renders the plist, bootstraps launchd, waits for `/healthz`, and prints the selected providers.

...
__main__.py (and remote_workstreams/ambient.py) are the only modules that wire concretes.
"""
...
from remote_workstreams.server import api, ws
from remote_workstreams.server.approvals import Approvals
from remote_workstreams.server.auth import LoginManager, PairingManager
from remote_workstreams.server.logs import log, setup_logging
from remote_workstreams.server.runtime import (
    ClientPush,
    ConvoBridge,
    ConvoReset,
    ConvoRuntime,
...
from remote_workstreams.substrate import Substrate

logger = logging.getLogger("remote_workstreams.server.http")
    ...
    async def get_response(self, path: str, scope) -> Response:
        try:
            ...
            try:
                ...
                return PlainTextResponse("remote-workstreams: PWA not built", status_code=404)
...
def create_app(
    ...
    *,
    ...
    app = FastAPI(title="remote-workstreams", lifespan=lifespan)

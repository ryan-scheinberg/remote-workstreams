"""The /ws endpoint. Speaks remote_workstreams/protocol.py exactly: first text frame must
...
from remote_workstreams.server.auth import LoginManager
from remote_workstreams.server.logs import log
from remote_workstreams.server.runtime import ConvoRuntime

logger = logging.getLogger("remote_workstreams.server.ws")
...
async def _handle(runtime: ConvoRuntime, conn: WSConnection, text: str) -> None:
    ...
    if isinstance(msg, protocol.TextInput):
        ...
        runtime.set_muted(msg.muted)
    elif isinstance(msg, protocol.Hush):
        runtime.set_hushed(msg.muted)

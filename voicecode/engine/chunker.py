...
Feed stream deltas in; complete sentences come out as soon as their boundary
arrives, so the first chunk reaches TTS at the first sentence boundary instead
of at end-of-response. Sentences shorter than `min_chars` are merged with the
next one so TTS isn't fed choppy fragments like "Sure.".
"""
